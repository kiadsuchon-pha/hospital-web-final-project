...
window.loadScheduleManager = async () => {
    ...
    if (!gridBody) return;
    gridBody.innerHTML = `<tr><td colspan="6" align="center">⏳ กำลังโหลด...</td></tr>`;
    const filterDeptId = document.getElementById('schedFilterDept')?.value || "";
    try {
        const [deptSnap, opdSnap, docSchSnap, docSnap, locSnap] = await Promise.all([getDocs(collection(db, "departments")), getDocs(collection(db, "opd_schedules")), getDocs(collection(db, "doctor_schedules")), getDocs(collection(db, "doctors")), getDocs(collection(db, "locations"))]);
        let depts = deptSnap.docs.map(d => ({ id: d.id, ...d.data() })).filter(d => d.is_active !== false);
        if (filterDeptId) depts = depts.filter(d => d.id === filterDeptId);
        const docs = {}; docSnap.forEach(d => docs[d.id] = getDoctorName(d.data()));
        const locs = {}; locSnap.forEach(d => locs[d.id] = d.data().name);
        ...
        opdSnap.forEach(doc => { 
            ...
            if (!map[d.dept_id][d.day]) map[d.dept_id][d.day] = {}; 
            map[d.dept_id][d.day][d.shift] = { opdData: d, assignments: [] }; 
        });
        
        docSchSnap.forEach(doc => { 
            const d = doc.data(); 
            if (map[d.dept_id]?.[d.day]?.[d.shift]) { map[d.dept_id][d.day][d.shift].assignments.push({ id: doc.id, ...d }); }
        });

        let html = ""; const days = ["จันทร์", "อังคาร", "พุธ", "พฤหัส", "ศุกร์"];
        depts.forEach(dept => {
            html += `<tr><td style="font-weight:bold;"><i class="fa-solid ${dept.icon || 'fa-hospital'}"></i> ${dept.name}</td>`;
            days.forEach(day => {
                html += `<td class="sched-cell">`;
                ['1', '2'].forEach(shift => {
                    const data = map[dept.id]?.[day]?.[shift];
                    ...
                    if (data) {
                        html += `<div style="font-size:0.75rem; color:#666; display:flex; justify-content:space-between; align-items:center; margin-bottom:5px;">
                            <span><i class="fa-solid fa-location-dot"></i> ${locs[data.opdData.location_id] || '-'}</span>
                        ...
                        if (data.assignments.length > 0) {
                            data.assignments.forEach(assign => {
                                const docNameForLog = docs[assign.doc_id]?.replace(/'/g, "\\'") || "แพทย์";
                                html += `<div class="doc-assigned" style="margin-top:4px;">
                                    <div class="text-truncate" title="${docs[assign.doc_id]}">${docs[assign.doc_id]}</div>
                                    <i class="fa-solid fa-xmark btn-remove-doc" onclick="removeDoctor('${assign.id}', '${docNameForLog}', '${dept.name}', '${day}')"></i>
                                </div>`;
                            });
                        }
